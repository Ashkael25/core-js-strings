...
function removeFirstOccurrences(str, value) {
  const first = str.indexOf(value);
  if (first === -1) {
    return str;
  }
  return str.slice(0, first) + str.slice(first + value.length);
}
...
function isPalindrome(str) {
  const poly = str.toLowerCase().replace(/\s|[,.!?"/-]/g, '');
...
function reverseWords(str) {
  ...
  for (let i = 0; i < string.length; i += 1) {
    res.push(string[i].split('').reverse().join(''));
  }
  return res.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if ((num >= 65 && num < 78) || (num >= 97 && num < 110)) num += 13;
    else if ((num >= 78 && num <= 90) || (num >= 110 && num <= 122)) num -= 13;
